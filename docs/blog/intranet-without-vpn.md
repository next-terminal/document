...
> In Next Terminal's case, gateway reverse tunnel + per-asset authorization + session audit are available out of the box and cover all four points above; see [Security Gateway](/docs/usage/agent-gateway), [Assets](/docs/usage/asset) and [Asset Access](/docs/usage/access) for specifics. Other bastions follow the same approach, differing mainly in UI.

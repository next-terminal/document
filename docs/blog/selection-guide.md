...
The metric that matters is "time from zero to first asset online": after initialization, create an SSH asset in [asset management](/docs/usage/asset), authorize it, and verify via [asset access](/docs/usage/access) — the whole loop runs in about 10 minutes.
...
- **SSH/RDP/VNC** are table stakes today. Next Terminal covers them via [RDP proxy server](/docs/usage/rdp-server) (use your local mstsc/RDP client with audit preserved) and [SSH proxy server](/docs/usage/ssh-server) for local terminals and automation.
- **Network traversal**: For multi-site or VPC-isolated assets, Next Terminal's [security gateway](/docs/usage/agent-gateway) runs as an agent to bridge private segments without opening every asset port at the edge. See [security gateway configuration](/docs/usage/agent-gateway-config).
- **Web and databases**: Next Terminal publishes internal systems via [web assets](/docs/usage/website) with [HTTPS mTLS](/docs/usage/mtls) for zero-trust exposure, and audits database sessions via [database audit](/docs/usage/database).
...
- **Authorization**: All three support user/group to asset/group mapping. Next Terminal keeps it direct — configure in assets and authorization, then rely on [Audit Logs](/docs/usage/audit) for session recording, command audit, and replay.
- **Session audit**: Verify three things before you buy — full-protocol recording, search by user/asset/time, and replay to pinpoint risky operations. See [Audit Logs](/docs/usage/audit) and the session management docs.
- **Identity hardening**: Add [passkey](/docs/usage/passkey) and [2FA (TOTP)](/docs/usage/otp), or federate via the [OIDC identity server](/docs/usage/oidc_server) to your corporate IdP.
...
- **Terminal experience**: The web terminal works out of the box; heavy local users can pair it with [Termark](/docs/usage/termark) for a native-terminal feel.
- **Stability**: For production, design database, storage, and ingress availability together, and review [real client IP](/docs/install/real-ip) handling to avoid proxy-related session or audit issues.

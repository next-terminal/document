...
This matters most on machines where untrusted users can log in, such as a public jump host: the administrator can close agent forwarding entirely and force users onto `ProxyJump` or a centralized entry. It is the same philosophy as `AllowTcpForwarding` for server-side port forwarding (covered in the [SSH Port Forwarding](/docs/blog/ssh-port-forwarding) piece) — turn the dangerous capability off by default on the server rather than counting on every client to behave. The difference is what each one blocks: `AllowAgentForwarding` stops the exposure of signing capability, while `AllowTcpForwarding` stops tunnel construction; a machine that allows proxy hops to support `-J` does not therefore need agent forwarding, and the two should be weighed separately.
...
That is exactly the pattern of a centralized access point like a bastion host. Next Terminal, an open-source access gateway of this kind, converges proxying, admission, and asset credential management into one entry; when reaching an internal asset, the credential is held server-side and injected on demand, independent of any client-side `ssh -A`. Paired with session recording and auditing (see [Audit Logs](/docs/usage/audit)), who authenticated into which asset and when is traceable; the specifics of centralized proxying and admission are in [SSH Proxy Server](/docs/usage/ssh-server) and [Access Assets](/docs/usage/access), with unified entries across networks in [Agent Gateway](/docs/usage/agent-gateway) and [SSH Gateway](/docs/usage/ssh-gateway).
...
Stepping back, agent-forwarding abuse is rarely one person's carelessness; more often the default of "forward if it works" has made the wrong decision on their behalf. Changing the default from "works" back to "safe" outperforms teaching every individual to remember to turn it off.

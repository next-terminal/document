...
When multiple team members need database access, SSH tunnels lack unified management and operation auditing — a classic use case for [database audit](/docs/usage/database).
...
> Combining with a [security gateway](/docs/usage/agent-gateway) further reduces database exposure by whitelisting only internal network access.

...
> When connecting through a bastion host, credentials are managed in [Assets](/docs/usage/asset). Use "Test Connection" on the bastion first to tell whether the failure is between bastion and target or between client and bastion.
...
> With [RDP Proxy Server](/docs/usage/rdp-server), you do not need to expose 3389 for every Windows host to the internet — only allow 3389 from the bastion to the internal hosts, and optionally front it with a [Security Gateway](/docs/usage/agent-gateway). See [Container Installation](/docs/install/container-install) for deployment.
...
> If the black screen only happens on a specific path, look up the code in [RDP/VNC Error Codes](/docs/usage/error-codes) to distinguish codec vs. session vs. network faults without reinstalling drivers blindly.
...
Opening 3389 host by host and managing passwords and firewall rules per machine does not scale and is hard to audit. Next Terminal v3.8.0 consolidates RDP into two segments — "user → bastion → asset": 3389 is only open inside the private network to the bastion and [Security Gateway](/docs/usage/agent-gateway); the internet only sees the bastion's proxy port. Credentials are vaulted in [Assets](/docs/usage/asset) and every session is recorded through [RDP Proxy Server](/docs/usage/rdp-server) — connectable, manageable and auditable. For deployment see [Container Installation](/docs/install/container-install) and verify the path with [Asset Access](/docs/usage/access) once the five steps above pass.

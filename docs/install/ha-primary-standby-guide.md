...
A database major-version migration cannot be completed by swapping images. See [PostgreSQL 16 to 18 Migration](/docs/install/postgresql-16-to-18) when needed.
...
See the full [`config.yaml` Reference](/docs/install/config-desc). Confirm both files are identical:
...
See [Real Client IP](/docs/install/real-ip) for trusted-proxy configuration. Do not set `IpTrustList` to `0.0.0.0/0` without a specific trust-boundary reason.
...
1. Create and download a [Backup and Restore](/docs/usage/backup) backup; separately protect `drive`, local recordings, and `config.yaml`.

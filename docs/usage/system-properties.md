...
Values and available keys can change between releases. Before using automation, confirm a key with `nt config get <key>` on the target version. See [CLI Reference](/docs/usage/cli) for command syntax.

::: danger Do not delete the entire `property` table
Deleting the table resets unrelated authentication, mail, recording, gateway and integration settings. Use `nt config get` and `nt config set` to inspect or repair a specific key. Create a [system backup](/docs/usage/backup) before changing configuration, and edit the database directly only when you have a tested restore path.
:::

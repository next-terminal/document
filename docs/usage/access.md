...
- The current account has [Resource Authorization](/docs/usage/authorization).
...
- [SSH Assets](/docs/usage/ssh): terminal, SFTP filesystem, snippets, monitoring and AI assistant.
- [Windows / RDP Assets](/docs/usage/rdp): desktop, clipboard, key combinations, network drive and RemoteApp.
- [VNC and Telnet Assets](/docs/usage/vnc-telnet): graphical VNC desktop or Telnet terminal.
- [Web Assets](/docs/usage/website): authenticated and authorized access to internal sites.
- [Database Audit](/docs/usage/database): database-client proxy access rather than a browser SQL session.
...
SSH file management directly operates the target SFTP filesystem. RDP uses Next Terminal storage and a Windows mapped drive. RDP/VNC clipboard availability depends on strategy and browser permission. See [File Management](/docs/usage/file-management).
...
4. For RDP/VNC, consult [Error Codes](/docs/usage/error-codes).
5. Review access and session logs.

For native clients, use the [SSH Proxy Server](/docs/usage/ssh-server), [RDP Proxy Server](/docs/usage/rdp-server) or [Termark](/docs/usage/termark).

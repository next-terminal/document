...
正常配置参阅 [Windows / RDP 资产](/zh/usage/rdp)和 [VNC 与 Telnet 资产](/zh/usage/vnc-telnet)。错误码含义参阅 [RDP/VNC 错误码](/zh/usage/error-codes)。
...
完整步骤参阅 [RDP 黑屏或连接失败排查](/zh/blog/rdp-black-screen-failed)。
...
文件传输流程参阅 [Windows 文件管理](/zh/usage/rdp#windows-文件管理)。
...
- 检查 VNC Server 日志和 [RDP/VNC 错误码](/zh/usage/error-codes)。

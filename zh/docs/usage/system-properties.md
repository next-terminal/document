...
配置键和取值可能随版本变化。用于自动化脚本前，请先在目标版本中通过 `nt config get <key>` 确认该配置项。命令格式参见[命令行工具参考](/zh/docs/usage/cli)。

::: danger 不要删除整个 `property` 表
删除整张表会同时重置认证、邮件、录像、网关和外部集成等无关配置。应优先使用 `nt config get` 和 `nt config set` 检查或修复单个配置项。修改前先完成[系统备份](/zh/docs/usage/backup)；只有具备经过验证的恢复方案时，才考虑直接修改数据库。
:::

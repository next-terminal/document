...
- 管理员已经创建资产并测试成功。
- 当前账号已通过[资源授权](/zh/docs/usage/authorization)获得访问权限。
...
- [SSH 资产](/zh/docs/usage/ssh)：终端、SFTP 文件系统、命令片段、状态监控和 AI 助手。
- [Windows / RDP 资产](/zh/docs/usage/rdp)：远程桌面、剪贴板、组合键、网络驱动器和 RemoteApp。
- [VNC 与 Telnet 资产](/zh/docs/usage/vnc-telnet)：VNC 图形桌面或 Telnet 字符终端。
- [Web 资产](/zh/docs/usage/website)：在通过认证和授权后访问内部网站。
- [数据库审计](/zh/docs/usage/database)：使用数据库客户端通过代理接入，不在此工作区内建立 SQL 会话。
...
具体步骤参阅[文件管理](/zh/docs/usage/file-management)。
...
4. 仅 RDP/VNC 失败时，查看 [RDP/VNC 错误码](/zh/docs/usage/error-codes)。
...
- [Termark 本地客户端](/zh/docs/usage/termark)

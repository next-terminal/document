...
保存资产并完成管理员连接测试后，通过[资源授权](/zh/docs/usage/authorization)分配给用户或部门。

| 接入方式 | 适合场景 |
| --- | --- |
| 浏览器远程桌面 | 无需安装客户端，适合临时访问和统一工作区 |
| [RDP 代理服务器](/zh/docs/usage/rdp-server) | 使用 Windows 远程桌面等本地客户端，保留原生交互体验 |
| [Termark](/zh/docs/usage/termark) | 在桌面客户端中统一查看和打开已授权的 SSH/RDP 资产 |
...
文件按钮是否显示由访问策略控制。更多操作和权限说明参阅[文件管理](/zh/docs/usage/file-management)。
...
参阅 [RDP/VNC 错误码](/zh/docs/usage/error-codes)和 [RDP 黑屏排查](/zh/docs/blog/rdp-black-screen-failed)。不要在不理解影响时长期关闭证书或安全校验。

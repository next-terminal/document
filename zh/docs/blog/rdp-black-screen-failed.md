...
> 堡垒机场景下口令由[资产管理](/zh/docs/usage/asset)托管，先在堡垒机侧“测试连接”区分是堡垒机到目标还是客户端到堡垒机的问题。
...
> 使用 [RDP 代理服务器](/zh/docs/usage/rdp-server)时仅需放行堡垒机到内网的 3389，配合[安全网关](/zh/docs/usage/agent-gateway)收敛暴露面，部署见[容器安装](/zh/docs/install/container-install)。
...
> 仅特定链路黑屏可对照 [RDP/VNC 错误码](/zh/docs/usage/error-codes)定位编解码或会话错误。
...
逐台开放 3389 难以审计。Next Terminal v3.8.0 将链路收敛为“用户—堡垒机—资产”：3389 仅对堡垒机与[安全网关](/zh/docs/usage/agent-gateway)开放，外网只暴露代理端口；凭证由[资产管理](/zh/docs/usage/asset)托管，通过 [RDP 代理服务器](/zh/docs/usage/rdp-server)实现可连可审。部署参考[容器安装](/zh/docs/install/container-install)，接入后用[资产访问](/zh/docs/usage/access)验证。

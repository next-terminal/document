...
迁移数据库主版本不能只替换镜像。需要迁移时参阅 [PostgreSQL 16 迁移到 18](/zh/install/postgresql-16-to-18)。
...
完整字段说明参阅 [`config.yaml` 配置参考](/zh/install/config-desc)。在两个节点检查文件内容一致：
...
真实客户端 IP 的信任范围参阅[获取真实 IP](/zh/install/real-ip)。不要把 `IpTrustList` 无条件设置成 `0.0.0.0/0`。
...
1. 按[系统备份与恢复](/zh/usage/backup)创建并下载备份，同时备份 `drive`、本地录像和 `config.yaml`。

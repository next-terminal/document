...
See [SSH Assets](/usage/ssh) for normal configuration. This page focuses on failures.
...
Use the asset alias, verify its syntax, confirm user authorization, ensure the protocol is SSH, and confirm that the proxy server is enabled. See [SSH Proxy Server](/usage/ssh-server).

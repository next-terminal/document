...
SSH directly operates the target filesystem; RDP operates Next Terminal storage through a Windows mapped drive. See [File Management](/usage/file-management).
...
The browser uploads to Next Terminal storage, not directly to `C:` or `D:`. Open the mapped drive in Windows and copy the file to its final disk. See [RDP and VNC](/faq/rdp-vnc#windows-does-not-show-the-mapped-drive) if the drive is absent.

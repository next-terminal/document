...
for marker in ("hostname: 'https://docs.next-terminal.com'","transformHead","transformPageData","sitemap:"):
...
if (public/'robots.txt').exists() and 'https://docs.next-terminal.com/sitemap.xml' not in (public/'robots.txt').read_text(): errors.append('docs robots sitemap wrong')
try:
 ...
 if not urls or any(not u.startswith('https://docs.next-terminal.com/') for u in urls): errors.append('built sitemap host wrong')
